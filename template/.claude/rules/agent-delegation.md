...
{{IMPL_PATHS}} 配下のコードを変更するとき（規模・ファイル数を問わず）、
...
<!--
骨格ファイル。/template-setup がプロジェクトのレイヤー構成で埋める。
レイヤーの数だけ表の行を増やす（例: `src/client/` → client-impl-agent / `src/server/` → server-impl-agent）。
{{IMPL_PATHS}} … 委譲対象となる実装パスの列挙（例: 「`src/client/` または `src/server/`」）。
-->
...
- テストファイル（{{TEST_FILE_PATTERN}}）も含めて agent 内で完結させる
