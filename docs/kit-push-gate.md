...
| 構成要素 | 中身 | 持ち主 |
|---|---|---|
| `MANDATORY_SCOPE` | `CLAUDE.md`（Claude への指示書）、`.github/`・`.githooks/`（ゲート定義）、`template/`・`scripts/`（scaffold で外へ出る経路）、宣言ファイル自身 | フック（`guard-kit-push-verdict.cjs`）に組み込み |
...
**審査1回はエージェント起動ぶんの金と時間である。** 守るものが無い push に払わない。
...
フックは配布されるが、このファイルは配布されない。「無ければマニフェストだけ見る」という設計にすると、宣言を持たない配布先の kit で `template/` や `CLAUDE.md` が無審査で外へ出る。**宣言し忘れは必ず安全側（審査過剰）に倒すこと。**
...
README はプロジェクトの概要と最小限の用法に留めるものであり、設計文書ではない。配布もされない。**大袈裟な審査を通さずとも誤りが分かる程度にコンパクトに保つべきもの**であり、そこに審査コストを払うくらいなら、その内容を `docs/` に置いて README からは参照するのが正しい。

`CLAUDE.md` はスコープに残す。こちらは Claude への指示書であり、実装とずれると挙動そのものが変わる。
